	...
	scan() {
		...
		if(messages_a_afficher.length > 0) {
            ...
            if (isTextareaFocused) {
                // On mémorise la distance par rapport au bas de la page AVANT l'animation
                distanceFromBottom = document.documentElement.scrollHeight - $(window).scrollTop();
                TL.log(`Zone de texte active. Verrouillage activé. Distance au bas: ${distanceFromBottom}px.`);
            }
			...
			setTimeout(() => {
                ...
                if (isTextareaFocused) {
                    ...
                    // On compense le décalage pour que la zone de texte ne bouge pas à l'écran
                    $(window).scrollTop(newScrollTop);
                    TL.log(`Scroll compensé à la position ${newScrollTop}px pour maintenir le focus.`);
                }
                

				if(maj) {
					this.maj();
                    // On ne scrolle automatiquement que si la zone de texte n'est PAS active.
...
class Favicon {
   ...
   maj(txt) {
        this.pendingText = txt; if (!this.imageLoaded) { return; } this.clear();
        if (txt && txt !== '') {
            ...
            this.context.shadowColor = 'transparent'; this.context.shadowBlur = 0; this.context.shadowOffsetX = 0; this.context.shadowOffsetY = 0;
        }
        this.replace();
    }
...
class TopicLive {
	constructor() {
		this.instance = 0; this.ongletActif = !document.hidden; this.unreadMessageAnchors = [];
	}
